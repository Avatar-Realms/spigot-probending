 ...
 */
public class TournamentUnregisterCommand extends ProbendingCommand {

    public TournamentUnregisterCommand() {
        this.command = "unregister";
        this.aliases.add("unreg");
    }
    ...
    public boolean execute(CommandSender sender, List<String> args) throws ProbendingException {
        ...
        if (!tour.isRegistered(team)) {
            throw new ProbendingException("error.command.team.notregistered");
        }
        tour.unregister(team);
        String msg = Messages.get("probending.team.unregistered").replace("{TEAM}", team.getName());
    ...
    public void printUsage(CommandSender sender) {
        sender.sendMessage(ChatColor.AQUA + "/probending unregister <TEAM_NAME>");
    }
}
